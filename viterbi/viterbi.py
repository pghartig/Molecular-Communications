...
def viterbi_reimplemented(transmit_alphabet, channel_output, channel_information):
    ...
    for i in range(channel_output.shape[1]):
        ...
        if i < channel_information.shape[1] - 1:
            ...
            for state in range(num_states):
                # symbol =\
                #     np.argmin((metric_vector[state * alphabet_size : (state + 1) * alphabet_size]) +
                #               survivor_paths_costs[state * alphabet_size : (state + 1) * alphabet_size] )
                test = survivor_paths_costs[state * alphabet_size: (state + 1) * alphabet_size]
                symbol = np.argmin(survivor_paths_costs[state * alphabet_size : (state + 1) * alphabet_size])
                survivor_paths[state, i] = transmit_alphabet[symbol]
                survivor_paths_costs[state, 0] += metric_vector[symbol]
