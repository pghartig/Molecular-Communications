...
def viterbi_output(transmit_alphabet, channel_output, channel_information):
    ...
    for detected_symbol in channel_output:
        ...
        metric_vector = gaussian_channel_metric(survivor_paths, i, transmit_alphabet, detected_symbol, channel_information)
        for state in range(num_states):
            survivor_paths[i, state] = transmit_alphabet[np.argmin((metric_vector[state*alphabet_size:(state+1)* alphabet_size]))]
        i += 1
