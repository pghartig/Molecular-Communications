...
\frametitle{Viterbi Setup}
	Probability of transmit signal given the received signal
	\begin{equation}
		Pr(\mathbf{x}|\mathbf{y})
	\end{equation}
	For channels without intersymbol interference
	\begin{equation}
...
\frametitle{Viterbi Setup Continued}
	Given a finite channel length and finite symbol constellation, sequence decoding can be formalized as 
	\begin{equation}
	\begin{array}{ll}
		\underset{\mathbf{x}}{\mathrm{maximize}} &\prod_{\mathrm{i=1}}^{\mathrm{N}}Pr(x_{\mathrm{i}}|y_{\mathrm{i}}) 
	\end{array}
	\end{equation}
\end{frame}
