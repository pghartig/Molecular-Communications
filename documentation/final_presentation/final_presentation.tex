...
The true state of the channel, $s[k]$, is hidden. we attempt to estimate this state based the sampled signal $y[k]$.
\begin{itemize}
\item 
	Observations are made of some channel in a point-to-point communication system.
\item 
	For each observation, this channel takes on a state $s[k]$. 
\item 
...
Over many observations, a sequence of observations $\mathbf{y}$ are made on the corresponding set of channel state $\mathbf{s}$
...
\end{figure}
\pause
For a channel represented by an LTI system, the state is determined by the transmitted information $\mathbf{x}$.
\end{frame}

\begin{frame}
\frametitle{Estimating the True Channel State}
We attempt to estimate the true sequence of channel states , $\mathbf{s}$, is hidden from us but we attempt to estimate this state based the sampled signal $y[k]$.
\pause
\end{frame}
...
Include decomposition and example for LTI channel. 
\end{frame}

\begin{frame}
\frametitle{Utilizing the General Definition of State}
Include decomposition and example for LTI channel. 
\end{frame}

\begin{frame}
\frametitle{Something about reduced state}
Include decomposition and example for LTI channel. 
\end{frame}
