...
def symbol_error_rate_mc_data(detected_symbols, input_symbols,channel_length):
    check = detected_symbols.astype('float64')
    ser = np.sum(np.not_equal(check[:input_symbols.size], input_symbols.astype('int32'))) / input_symbols.size
    return ser
